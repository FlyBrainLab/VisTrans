...
class PointMap(with_metaclass(ABCMeta, object)):
...
class PointMapWithDirection(with_metaclass(ABCMeta, object)):
    """ Interface of mapping a point from one surface to another
        (hence the 2 parameters)
    """
    # __metaclass__ = ABCMeta
...
class EyeToImagePointMap(with_metaclass(ABCMeta, PointMap)):
