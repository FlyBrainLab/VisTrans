...
window.renewCircuit = function () {
    if (window.IOName in window.fbl.experimentConfig) {
        if ('disabled' in window.fbl.experimentConfig[window.IOName]) {
            ...
            for (var key in window.fbl.experimentConfig['retina']) {
                console.log('Checking for ', key);
                var keyName = key;
                $(window.neuron_selector).each(function (index, value) {
                    var configName = $(value).attr('id');
                    var omma = configName + '-' + window.customCircuitAttributes['ommatidium_num'];
                    if (keyName == omma) {
                        console.log('Found previous setting for and updated ' + omma + '.');
                        window.simModels[$(value).attr('simID')] = window.fbl.experimentConfig['retina'][keyName];
                        window.simNames[index] = window.fbl.experimentConfig['retina'][keyName]['params']['name'];
                    }
                });
            }
...
window.IOName = 'retina';
if (window.IOName in window.fbl.experimentConfig) {
}
else {
    window.fbl.experimentConfig['retina'] = {};
}

window.fbl.experimentConfig['retina'].updated = [];
window.fbl.experimentConfig['retina'].disabled = [];
...
// var ommatidiium_data_set = false;
...
function toggleByID(a, echo = true) {
    ...
    if (neu.attr("visible") == "true") {
        ...
        window.fbl.experimentConfig['lastLabel'] = a + '-' + window.customCircuitAttributes['ommatidium_num'];
    } else {
    ...
    if (echo == true) {
        ...
        // b = b.replace("alpha", "a");
        if (b.indexOf('R') > -1)
            window._neuGFX.mods.FlyBrainLab.sendMessage({ messageType: 'NLPquery', query: hideOrShow + " $" + b + '-' + window.customCircuitAttributes['ommatidium_num'] + "$"});
        else
            window._neuGFX.mods.FlyBrainLab.sendMessage({ messageType: 'NLPquery', query: hideOrShow + " $" + b + '-' + window.customCircuitAttributes['ommatidium_num'] + "$" });
    }
    window.updateCircuit();
...
window._neuGFX.mods.FlyBrainLab.addFBLPath("Ommatidium" + window.customCircuitAttributes['ommatidium_num'], function () { });
...
window.getModelData = function (callback, simModel) {
    if ($('.NeuGFX-overlay').length) {
        ...
        var omma = configName + '-' + window.customCircuitAttributes['ommatidium_num'];
        window.fbl.experimentConfig['retina'][omma] = window.simModels[simID];
...
window.createOverlay = function (simID, simModel) {
        ...
        for (var modelFamily in _NKModels) {
            for (var model in _NKModels[modelFamily]) {
                if (model == modelName) {
                    ...
                    var omma = configName + '-' + window.customCircuitAttributes['ommatidium_num'];
                    console.log(omma);
                    window.fbl.experimentConfig['retina'][omma] = JSON.parse(JSON.stringify(window.simModels[simID]));
    ...
    $(document).on('click', function (event) {
        ...
        window.getModelData(function () {
            ...
            var omma = configName + '-' + window.customCircuitAttributes['ommatidium_num'];
            window.fbl.experimentConfig['lastUpdated'] = omma;
            $('.NeuGFX-overlay').remove();
        }, window.simModels[simID]);
    });
    var configName = $(simIDs[simID]).attr('tooltip-data').split(" :: ")[0];
    var omma = configName + '-' + window.customCircuitAttributes['ommatidium_num'];
    if (!(window.fbl.experimentConfig['retina'].updated.includes(omma))) {
        console.log(window.fbl.experimentConfig['retina'].updated);
        console.log(configName);
        window.fbl.experimentConfig['retina'].updated.push(omma);
    }
}
    ...
    MembraneModels: {
        ...
        PhotoreceptorModel:
        {
            ...
            params: {num_microvilli: 30000,
                     ...
                     optic_axis_elev: 0.0,
                     optic_axis_azim: 0.0,
                     acceptance_angle: 5.0
            },
            names: {'V': 'Membrane Potential - Set Initial Value (mV)',
                    'num_microvilli': 'Number of microvilli per photoreceptor',
                    'elev_3d': 'elevation angle of the position of photoreceptor',
                    'azim_3d': 'azimuthal angle of the position of photorecepor',
                    'optic_axis_elev': 'elevation angle of the optical axis',
                    'optic_axis_azim': 'azimuthal angle of the optical axis',
                    'acceptance_angle': 'acceptance angle of the photoreceptor'
                }
        }
    },
    SynapseModels: {
